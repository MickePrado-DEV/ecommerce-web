...
export function RegisterDriverForm() {
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    try {
      ...
      toast.success('Cuenta repartidor creada');
  ...
  return (
    <Card className="mx-auto w-full max-w-md border-white/10 bg-white/5">
      <CardHeader>
        <CardTitle>Registro repartidor</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div><Label>Licencia</Label><Input {...form.register('licenseNumber')} /></div>
          <div><Label>Placa vehículo</Label><Input {...form.register('vehiclePlate')} /></div>
          <div><Label>Contraseña</Label><Input type="password" {...form.register('password')} /></div>
          <Button type="submit" className="w-full">Registrarse</Button>
          <p className="text-center text-sm text-zinc-400">
            <Link href="/login" className="text-violet-400 hover:underline">Iniciar sesión</Link>
          </p>
        </form>
      </CardContent>
    </Card>
