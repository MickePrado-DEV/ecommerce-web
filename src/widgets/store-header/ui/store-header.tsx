    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-zinc-950/90 backdrop-blur">
      <div className="container mx-auto flex h-16 items-center justify-between gap-4 px-4">
        ...
        <nav className="hidden gap-6 md:flex">
          ...
          <Link href="/search" className="text-sm hover:text-violet-400">
            Buscar
          </Link>
          {user && (
            <Link href="/orders" className="text-sm hover:text-violet-400">
              Pedidos
            </Link>
          )}
          {user && (
            <Link href="/wishlist" className="text-sm hover:text-violet-400">
              Favoritos
            </Link>
          )}
        </nav>
