...
function LatestProductCard({ product }: { product: ProductListItemDto }) {
  return (
    <article className="flex flex-col overflow-hidden rounded-xl border border-gray-800 bg-gray-900/80 transition hover:border-purple-500/40">
      <div className="relative aspect-[4/3] bg-gray-950">
        {product.primaryImage ? (
          ...
          <div className="flex h-full items-center justify-center text-sm text-gray-600">Sin imagen</div>
        )}
...
export function LatestProducts({ products }: { products: ProductListItemDto[] }) {
  if (!products.length) return null;

  return (
    <section>
      <h2 className="text-2xl font-bold text-white sm:text-3xl">Últimos Productos</h2>
      <p className="mt-2 text-gray-400">Explora las novedades más recientes de nuestro catálogo.</p>
      <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {products.map((p) => (
          <LatestProductCard key={p.id} product={p} />
        ))}
      </div>
    </section>
  );
